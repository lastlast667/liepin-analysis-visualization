...
from config.settings import RAW_DATA_DIR, PROCESSED_DATA_DIR, INTERMEDIATE_DATA_DIR, MODEL_DIR
import clean
import preprocess
import label
import tokenizer
...
file_handler = logging.FileHandler(PROJECT_ROOT / "pipeline.log", encoding="utf-8")
...
def main():
    ...
    主函数，用于运行完整分析管道
    """
    parser = argparse.ArgumentParser(description="运行完整分析管道")
    parser.add_argument(
        "--resume",
        type=str,
        default=None,
    ...
    if args.resume:
        resume_idx = STEP_NAMES.index(args.resume)
        start_idx = resume_idx
        logger.info(f"从步骤 [{args.resume}] 恢复执行")
    else:
        start_idx = 0
    ...
    for i in range(start_idx, len(STEP_NAMES)):
        ...
        if df is None and step != "raw":
            ...
            if ckpt and ckpt.exists():
                ...
                raise FileNotFoundError(f"缺少 {prev_step} 的 checkpoint: {ckpt}")
