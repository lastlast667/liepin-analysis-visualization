<template>
  <div class="space-y-6 animate-fade-in">
    ...
    <template v-if="currentView === 'list'">
      ...
      <!-- 推荐策略卡片 -->
      <div class="glass-card p-5 relative z-20">
        <div class="flex items-center justify-between gap-6">
          ...
          <div class="flex items-center gap-3 flex-shrink-0">
            <!-- 数量下拉 -->
            <div class="relative" ref="topKRef">
              <div @click="topKOpen = !topKOpen" class="flex items-center gap-2 cursor-pointer">
                <span class="text-sm text-gray-500 whitespace-nowrap">显示数量：</span>
                <div class="glass-input flex items-center justify-between gap-2 text-sm py-2 px-3 min-w-[60px]">
                  <span class="text-gray-300">{{ topK }}个</span>
                  <svg class="w-3.5 h-3.5 text-gray-500 transition-transform flex-shrink-0" :class="{ 'rotate-180': topKOpen }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                  </svg>
                </div>
              </div>
              <div v-if="topKOpen" class="absolute z-[100] mt-1 right-0 glass-card p-2 min-w-[80px] shadow-2xl">
                <div v-for="k in topKOptions" :key="k" @click="topK = k; topKOpen = false; fetchRecommendations()"
                     class="px-3 py-2 rounded-lg hover:bg-dark-700 cursor-pointer text-sm text-center"
                     :class="topK === k ? 'text-primary-400' : 'text-gray-300'">{{ k }}个</div>
              </div>
            </div>
            <!-- 策略按钮 -->
        ...
        <div v-else class="p-5">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div v-for="job in recommendations" :key="job.id"
                 ...
                 class="glass-card p-6 min-h-[210px] hover:border-primary-500/40 transition-all duration-300 cursor-pointer group hover:-translate-y-0.5 hover:shadow-lg">
              <div class="flex items-start justify-between gap-3">
                <div class="flex-1 min-w-0">
                  <!-- 岗位标题 + 行业标签 -->
                  <div class="flex items-center gap-3 mb-2">
                    <h3 class="text-xl font-semibold text-gray-200 group-hover:text-primary-400 transition-colors truncate">
                      {{ job.title }}
                    </h3>
                    <span v-if="job.companyIndustry" class="px-2 py-1 rounded text-sm bg-dark-700 text-gray-400 flex-shrink-0 leading-tight">{{ job.companyIndustry }}</span>
                  </div>
                  <!-- 公司名 · 城市 -->
                  <p class="text-lg text-gray-500 mb-3 truncate">{{ job.companyName }} · {{ job.locationCity }}</p>
                  <!-- 薪资 / 经验 / 学历 -->
                  <div class="flex items-center gap-3 text-sm text-gray-500 flex-wrap">
                    <span class="flex items-center gap-1 text-primary-400 font-medium text-lg">
                ...
                <div v-if="job.matchScore != null" class="text-center shrink-0">
                  <div class="w-16 h-16 rounded-full bg-gradient-to-br from-accent-500 to-accent-600 flex items-center justify-center">
                    <span class="text-xl font-bold text-white">{{ job.matchScore }}</span>
                  </div>
                  <span class="text-sm text-gray-500 mt-1 block leading-tight">匹配</span>
                </div>
                <!-- 收藏按钮 -->
                <button v-if="job.matchScore == null" @click.stop="toggleFavorite(job)"
                        class="p-2 rounded-lg hover:bg-dark-700 transition-all flex-shrink-0 self-start mt-0.5"
...
const topK = ref(12)
const topKOptions = [8, 12, 16]
const topKOpen = ref(false)
const topKRef = ref(null)
...
function handleClickOutside(e) {
  if (topKRef.value && !topKRef.value.contains(e.target)) topKOpen.value = false
}
